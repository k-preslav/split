...
export async function updateDataCollection(data) {
  if (!userDetails.userProfile?.dataCollectionDocumentId) {
    console.warn('No data collection document ID found in user profile.');
    return;
  }
  ...
  try {
    ...
    console.error('Error updating data collection document:', error, error.code);
    throw error;
  }
}

export async function getDataCollectionDocument() {
  const docId = userDetails.userProfile?.dataCollectionDocumentId;
  if (!docId) {
    console.warn('No data collection document ID found in user profile.');
    return null;
  }
  ...
  try {
    ...
    if (error.code === 404) {
      console.warn('Data collection document not found.');
    }

    console.error('Error fetching data collection document:', error, error.code);
    throw error;
  }
}
...
export async function updateUserLocale() {
  const dataCollectionDoc = await getDataCollectionDocument();
  if (dataCollectionDoc) {
    const savedLocale = dataCollectionDoc.userLocale;
    const currentLocale = Localization.getLocales()[0]?.regionCode || 'unknown';

    if (savedLocale !== currentLocale) {
      const res = await updateDataCollection({ userLocale: currentLocale });
      if (res) {
        console.log('Updated user locale');
      } else {
