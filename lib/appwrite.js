...
export async function sendChangePassword(overrideEmail=null) {
  ...
  try {
    await account.createRecovery(email, 'https://splitapp.loophole.site/pages/auth/change-password');
    return true;
  } catch (error) {
...
export async function sendVerify() {
  ...
  try {
    await account.createVerification('https://splitapp.loophole.site/pages/auth/verify-email');
