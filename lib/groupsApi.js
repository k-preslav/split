  ...
  try {
    const response = await databases.createDocument(
      ...
      ID.unique(),
      {
        ...
        groupCurrency: getCurrencyFromLocale(),
      }
    );
...
export async function updateGroup(groupId, updatedData) {
  try {
    ...
    if (response) {
      console.log("Group updated successfully", response.groupName);
      return response;
    }
  } catch (error) {
...
export async function getGroupsByOwnerId(ownerId) {
  if (!ownerId) {
    console.warn("No ownerId provided, cannot fetch groups.");
    return [];
  }
  ...
  try {
    ...
    if (response.documents.length > 0) {
      ...
      console.log("No owned groups found for ownerId:", ownerId);
      return [];
    }
  } catch (error) {
...
export async function getMemberGroupsByUserCode(userCode) {
  if (!userCode) {
    console.warn("No userCode provided, cannot fetch member groups.");
    return [];
  }
  ...
  try {
    const response = await databases.listDocuments(
      process.env.EXPO_PUBLIC_APPWRITE_DATABASE_ID,
      process.env.EXPO_PUBLIC_APPWRITE_GROUPS_COLLECTION_ID,
      [Query.contains('friendsCodes', userCode)]
    );

    if (response.documents.length > 0) {
      console.log("Member groups fetched successfully for userCode:", userCode);
      return response.documents;
    } else {
      console.log("No member groups found for userCode:", userCode);
      return [];
    }
  } catch (error) {
    console.error("Error fetching member groups by userCode:", error);
    return [];
  }
}
...
export async function uploadGroupImage(file) {
  ...
  return await storage.createFile(
    ...
    ID.unique(),
    {
      name: `groupImage_${ID.unique()}.jpg`,
      type: 'image/jpeg',
      size: 0,
      uri: file,
    },
