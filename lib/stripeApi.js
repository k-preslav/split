...
export async function connectToStripe(reqData) {
  try {
    ...
    const response = await fetch('https://splitapi.loophole.site/payments/create-stripe-connect-account', {
      method: 'POST',
      headers: {
...
export async function uploadStripeFile(file) {
  if (!file) {
    throw new Error('File is required to upload to Stripe');
  }

  console.log('Uploading file to Stripe:', file);
  ...
  try {
    ...
    const response = await fetch('https://splitapi.loophole.site/payments/stripe-upload-id-file', {
      method: 'POST',
      body: formData,
    });

    const data = await response.json();
    console.log('Stripe file uploaded');

    if (!response.ok) {
      throw new Error(data.error || 'Failed to upload file to Stripe');
    }
